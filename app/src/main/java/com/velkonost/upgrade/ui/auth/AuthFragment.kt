...
import com.velkonost.upgrade.databinding.FragmentAuthBinding
...
import com.velkonost.upgrade.ui.activity.main.ext.observeUserSettings
import com.velkonost.upgrade.ui.base.BaseFragment
import com.velkonost.upgrade.ui.view.SimpleCustomSnackbar
import com.velkonost.upgrade.util.ext.observeOnView
...
//                    updateUI(null)
    ...
    inner class Handler {
        fun signInClicked(v: View) {
            ...
            when {
                binding.email.text.isNullOrEmpty() -> EventBus.getDefault().post(ShowFailEvent("Укажите email"))
                binding.password.text.isNullOrEmpty() -> EventBus.getDefault().post(ShowFailEvent("Укажите пароль"))
                else -> login(
        ...
        fun signUpClicked(v: View) {
            ...
            when {
                binding.emailSignUp.text.isNullOrEmpty() -> EventBus.getDefault().post(ShowFailEvent("Укажите email"))
                binding.passwordSignUp.text.isNullOrEmpty() -> EventBus.getDefault().post(ShowFailEvent("Укажите пароль"))
                binding.passwordSignUp.text.toString().length < 6 -> EventBus.getDefault().post(ShowFailEvent("Пароль слишком короткий"))
                binding.passwordRepeatSignUp.text.isNullOrEmpty() -> EventBus.getDefault().post(ShowFailEvent("Укажите пароль повторно"))
                else -> register(
